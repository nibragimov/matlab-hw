...
close all
clf
handle_axes= axes('XLim', [-0.4,0.4], 'YLim', [-0.2,0.5], 'ZLim', [0,0.4]);
...
%change r and h of cylinder
length0= 0.2; radius0= 0.05;
%last array changes color
h(1)= link_box([0.08 0.08 length0], trf_viz_link0, [102/255 0/255 51/255], [102/255 0/255 51/255], 0.6); 
...
h(2)= link_cylinder(radius1, length1, trf_viz_link1, [1, 1, 0], [1, 1, 0] * 0.5, 0.6); 
...
h(3)= link_box([0.2, 0.02, 0.04], trf_viz_link1p2, [102/255 0/255 51/255], [102/255 0/255 51/255], 0.6); 
...
h(4)= link_sphere(0.03, trf_viz_link2,  [1, 1, 0], [1, 1, 0] * 0.5, 0.6); 
...
h(5)= link_cylinder(0.015, 0.1, trf_viz_link2p2, [102/255 0/255 51/255], [102/255 0/255 51/255], 0.6); 
plot_axes(trf_viz_link2p2, ' ', true, axis_length); % V_{2-2}

%% Link 2-3 
trf_viz_link2p3= make_transform([0, 0, 0], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(6)= link_sphere(0.02, trf_viz_link2p3, [1, 1, 0], [1, 1, 0] * 0.5, 0.6); 
plot_axes(trf_viz_link2p3, ' ', true, axis_length); % V_{2-3}
%% Link-3 
trf_viz_link3= make_transform([0, 0, -0.03], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(7)= link_cylinder(0.012, 0.06, trf_viz_link3, [102/255 0/255 51/255], [102/255 0/255 51/255], 0.6); 
...
h(8)= link_cylinder(0.006, 0.04, trf_viz_link3p2, [102/255 0/255 51/255], [102/255 0/255 51/255], 0.6); 
...
% the direction of rotation of the joint
...
%% Joint: Links 1,1_2: Prismatic
j1_2_translation_axis = [1,0,0]';
j1_2_translation = 0;

trf_link1p2_link1= make_transform([0, 0, 0], 0, 0, 0, trf_link1_joint1); 
trf_link2_joint12 = make_transform_prismatic(j1_2_translation_axis, j1_2_translation, trf_link1p2_link1);
make_child(trf_link2_joint12, trf_viz_link1p2);
...
trf_joint2_link1= make_transform([0.2, 0, 0], 0, 0, pi/2, trf_link2_joint12); 
trf_link2_joint2= make_transform_revolute(j2_rot_axis_j2, j2_rot_angle, trf_joint2_link1);
%trf_link2_joint3= make_transform_revolute(j2_rot_axis_j2, j2_rot_angle, trf_joint2_link1);
...
%% Joint: Links 2,2_3: Revolute
j2_3_rot_axis_j3= [0,0,1]';
j2_3_rot_angle= 0; % [-pi/2, pi/2]

trf_link2p3_joint23 = make_transform([0, -0.1, 0], 0, 0, 0, trf_link2_joint2);
trf_link3_joint23= make_transform_revolute(j2_3_rot_axis_j3, j2_3_rot_angle, trf_link2p3_joint23);
%plot_axes(trf_link3_joint23, 'L_{2-3}', false, axis_length); 
make_child(trf_link3_joint23, trf_viz_link2p3);
...
trf_joint3_link2= make_transform([0, -0.07, -0.025], pi/2+pi/10, 0, 0, trf_link3_joint23);
...
make_child(trf_joint3_link2, trf_viz_link3);
...
for q1=[linspace(0, -pi/2, 30), linspace(-pi/2, pi/2, 30), linspace(pi/2, 0, 30)]
    set(handle_axes, 'XLim', [-0.4,0.4], 'YLim', [-0.2,0.5], 'ZLim', [0,0.4]);
    trf_q1= makehgtform('axisrotate', j1_rot_axis_j1, q1);
    set(trf_link1_joint1, 'Matrix', trf_q1);
    drawnow;
    pause(0.02);
end
for a12=[linspace(0, -0.07, 30), linspace(-0.07, 0, 30)]
    set(handle_axes, 'XLim', [-0.4,0.4], 'YLim', [-0.2,0.5], 'ZLim', [0,0.4]);
    trf_a12= makehgtform('translate', j1_2_translation_axis *a12);
    set(trf_link2_joint12, 'Matrix', trf_a12);
    drawnow;
    pause(0.02);
end
...
for q23=[linspace(0, -pi/2, 30), linspace(-pi/2, pi/2, 30), linspace(pi/2, 0, 30)]
    set(handle_axes, 'XLim', [-0.4,0.4], 'YLim', [-0.2,0.5], 'ZLim', [0,0.4]);
    trf_q23= makehgtform('axisrotate', j2_3_rot_axis_j3, q23);
    set(trf_link3_joint23, 'Matrix', trf_q23);
    drawnow;
    pause(0.02);
end
...
q_init= 0.5*ones(6,1); % This leads to all joints being at 0.
